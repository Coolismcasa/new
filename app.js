/* ═══════════════════════════════════════════════════════════
   ...
   Auth + Firestore + Cart + Wishlist + Admin + Reviews
   ═══════════════════════════════════════════════════════════ */
...
const FALLBACK_PRODUCTS = [
  { id:'p1', name:'Classic Red Tee', cat:'shirts', price:2490, oldPrice:3200, tag:'New',
    ...
    desc:'A relaxed-fit tee cut from heavyweight 320 GSM cotton. Drop shoulders, ribbed collar, and a soft hand-feel.',
    sizes:['S','M','L','XL','XXL'],
    colors:[{name:'Red',hex:'#E33A3A'},{name:'Navy',hex:'#0B1A30'},{name:'Off-White',hex:'#F1EFE9'}],
    fabric:'100% Combed Cotton — 320 GSM', care:'Machine wash cold', sku:'CLM-SH-001', inStock:true },
  { id:'p2', name:'Royal Oxford Shirt', cat:'shirts', price:4990, oldPrice:null, tag:null,
    ...
    fabric:'100% Egyptian Cotton Oxford', care:'Machine wash warm', sku:'CLM-SH-002', inStock:true },
  { id:'p3', name:'Pleated Wide-Leg Trousers', cat:'pants', price:5890, oldPrice:6990, tag:'Bestseller',
    ...
    fabric:'Poly-wool blend', care:'Dry clean only', sku:'CLM-PT-001', inStock:true },
  { id:'p4', name:'Relaxed Linen Pants', cat:'pants', price:4290, oldPrice:null, tag:null,
    ...
    fabric:'100% European Linen', care:'Machine wash cold', sku:'CLM-PT-002', inStock:true },
  { id:'p5', name:'Moto Leather Jacket', cat:'jackets', price:18900, oldPrice:22500, tag:'Limited',
    ...
    fabric:'Full-grain sheep leather', care:'Wipe clean', sku:'CLM-JK-001', inStock:true },
  { id:'p6', name:'Navy Bomber Jacket', cat:'jackets', price:11900, oldPrice:null, tag:null,
    ...
    fabric:'Recycled nylon shell', care:'Machine wash cold', sku:'CLM-JK-002', inStock:true },
  { id:'p7', name:'Oversized Navy Hoodie', cat:'hoodies', price:6490, oldPrice:7990, tag:'Bestseller',
    ...
    fabric:'85% Cotton / 15% Poly', care:'Machine wash cold', sku:'CLM-HD-001', inStock:true },
  { id:'p8', name:'Fleece-Lined Zip Hoodie', cat:'hoodies', price:7290, oldPrice:null, tag:'New',
    ...
    fabric:'Cotton-poly blend', care:'Machine wash cold', sku:'CLM-HD-002', inStock:true }
];
...
function clearErrors(form) {
  if (form) form.querySelectorAll('[data-field]').forEach(f => f.classList.remove('err'));
}
...
async function loadCatalog() {
  try {
    ...
    console.warn('Categories load failed, using fallback', err);
    CATEGORIES = { ...FALLBACK_CATS };
  }
  ...
  try {
    ...
    console.warn('Products load failed, using fallback', err);
    PRODUCTS = [ ...FALLBACK_PRODUCTS ];
  }
}
...
function renderFooterCats() {
  ...
  ul.innerHTML = Object.keys(CATEGORIES).map(k =>
    `<li><a href="category.html?cat=${k}">${CATEGORIES[k].label}</a></li>`).join('');
}
...
function productCardHTML(p) {
  ...
  const outClass = p.inStock === false ? 'out-of-stock' : '';
  return `<article class="card ${outClass}" data-id="${p.id}">
    <div class="card-media">
      ${p.tag ? `<span class="tag">${p.tag}</span>` : ''}
      ${productVisualHTML(p)}
      <button class="wish-toggle ${wished}" data-wish="${p.id}" aria-label="Wishlist">
        <svg viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
      </button>
      <button class="quick-add" data-quick="${p.id}" ${p.inStock === false ? 'disabled' : ''}>${p.inStock === false ? 'Out of Stock' : 'Add to Bag'}</button>
    </div>
    <div class="card-body">
...
function loadLocalWishlist() {
  try { wishlist = JSON.parse(localStorage.getItem('coolism_wishlist') || '[]'); } catch (e) { wishlist = []; }
  updateWishlistBadge();
}
function saveWishlist() {
  try { localStorage.setItem('coolism_wishlist', JSON.stringify(wishlist)); } catch (e) {}
  updateWishlistBadge();
}
...
function toggleWishlist(id) {
  ...
  const detailWish = document.getElementById('detailWishBtn');
  if (detailWish && currentDetail.product && currentDetail.product.id === id) {
    detailWish.classList.toggle('wished', wishlist.includes(id));
  }
...
function renderWishlistDrawer() {
  ...
  if (!items.length) {
    body.innerHTML = `<div class="cart-empty">
      ...
      <a href="index.html#shop" class="btn btn-line" style="margin-top:16px">Browse Products</a>
    </div>`;
    return;
  }
  body.innerHTML = items.map(p => `
    <div class="cart-item">
      <div class="ci-thumb">${cartVisualHTML(p)}</div>
      <div class="ci-info">
        <h4>${p.name}</h4>
...
function addToCart(product, size, color, qty) {
  if (product.inStock === false) return toast('Out of stock');
...
function renderCart() {
  ...
  if (shipFill && shipMsg) {
    const pct = Math.min(100, (t / freeThreshold) * 100);
    shipFill.style.width = pct + '%';
    if (t >= freeThreshold) {
      shipMsg.textContent = '🎉 You got free shipping!';
      if (shipProgress) shipProgress.classList.add('free');
    } else {
      shipMsg.textContent = `Add ${money(freeThreshold - t)} more for free shipping`;
      if (shipProgress) shipProgress.classList.remove('free');
    }
  }
  ...
  if (!cart.length) {
    body.innerHTML = `<div class="cart-empty">
      ...
      <a href="index.html#shop" class="btn btn-line" style="margin-top:16px">Start Shopping</a>
    </div>`;
    return;
  }
  body.innerHTML = cart.map(i => `
    <div class="cart-item">
      <div class="ci-thumb">${cartVisualHTML(i)}</div>
      <div class="ci-info">
        <h4>${i.name}</h4>
...
function openDetail(id) {
  ...
  media.innerHTML = productVisualHTML(p, 'detail-placeholder');
  if (currentDetail.images.length > 1) {
    thumbs.innerHTML = currentDetail.images.map((img, i) =>
      `<div class="thumb ${i === 0 ? 'active' : ''}" data-thumb="${i}"><img src="${img}" alt="${p.name} ${i + 1}"></div>`
    ).join('');
  } else {
    thumbs.innerHTML = '';
  }

  document.getElementById('detailCat').textContent = getCat(p.cat).label;
  ...
  document.getElementById('detailFabric').textContent = p.fabric || '—';
  ...
  try {
    ...
    list.innerHTML = revs.map(r => {
      ...
      return `<div class="review-item">
        <div class="rv-head"><span class="rv-name">${r.userName || 'Anonymous'}</span><span class="rv-stars">${stars}</span></div>
        <p class="rv-text">${r.text || ''}</p>
        ${date ? `<p class="rv-date">${date}</p>` : ''}
      </div>`;
    }).join('');
  } catch (err) {
...
function performSearch(q) {
  ...
  if (!matches.length) {
    results.innerHTML = `<div class="search-empty">No products match "${q}"</div>`;
  ...
  if (u) {
    db.collection('users').doc(u.uid).get().then(snap => {
      if (snap.exists) {
        ...
        set('co-city', d.city); set('co-district', d.district); set('co-province', d.province);
      }
    }).catch(() => {});
  }
...
async function loadProfile(user) {
  ...
  try {
    ...
    set('pf-address', d.address);
    set('pf-city', d.city);
    set('pf-district', d.district);
    set('pf-province', d.province);
...
function renderProfileOrders(orders) {
  ...
  container.innerHTML = orders.map(o => {
    ...
    return `<div class="order-card">
      ...
      <div class="order-track">
        <div class="order-track-fill" style="width:${trackFillPct}%"></div>
        ${steps.map((s, i) => {
  ...
  container.querySelectorAll('[data-view-order]').forEach(btn => {
    btn.addEventListener('click', () => showOrderDetail(btn.dataset.viewOrder, orders));
  });
}
...
function renderProfileWishlist() {
  ...
  if (!items.length) {
    grid.innerHTML = `<div class="empty-state" style="grid-column:1/-1">
      ...
      <h3>Your wishlist is empty</h3><p>Heart products you love to save them here.</p>
    </div>`;
    return;
  }
...
function renderProductsTable(list) {
  ...
  tbody.innerHTML = list.map((p, i) => {
    ...
    const stock = p.inStock !== false ? '<span class="badge-yes">In Stock</span>' : '<span class="badge-no">Out</span>';
    return `<tr>
      <td>${i + 1}</td>
      <td>${thumb}</td>
      <td><b>${p.name}</b></td>
  ...
  tbody.querySelectorAll('[data-delete-product]').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      try {
        await db.collection('products').doc(btn.dataset.deleteProduct).delete();
...
function parseImages(str, baseUrl) {
  if (!str) return [];
  return str.split(',').map(s => s.trim()).filter(Boolean).map(fn =>
    fn.startsWith('http') ? fn : baseUrl + fn);
}
...
async function saveProduct(e) {
  ...
  const ok = [
    setErr(document.getElementById('p-name'), name.length < 2),
    setErr(document.getElementById('p-price'), !price || price < 0)
  ].every(Boolean);
  ...
  const images = parseImages(imagesRaw, baseUrl);
...
function renderOrdersTable(list) {
  ...
  tbody.querySelectorAll('[data-view-order]').forEach(btn => {
    btn.addEventListener('click', () => showAdminOrder(btn.dataset.viewOrder));
  });
}
...
function showAdminOrder(orderId) {
  ...
  document.getElementById('aoBody').innerHTML = `
    <div class="order-detail-block">
      ...
      <div class="row"><b>Full Name</b><span>${o.fullName || '—'}</span></div>
  ...
  modal.querySelectorAll('[data-status]').forEach(btn => {
    btn.classList.toggle('active-status', btn.dataset.status === (o.status || 'pending'));
  });
...
function renderUsersTable(list) {
  ...
  tbody.querySelectorAll('[data-view-user]').forEach(btn => {
    btn.addEventListener('click', () => showAdminUser(btn.dataset.viewUser));
  });
}
...
function showAdminUser(uid) {
  ...
  const ordersHtml = userOrders.length
    ? userOrders.map(o => `
        <div class="order-item">
          <div><div class="nm">${o.orderId} · ${money(o.total)}</div><div class="vr">${o.itemCount || 0} items · ${o.city || '—'}</div><div class="qt">${o.createdAt?.toDate ? o.createdAt.toDate().toLocaleDateString() : '—'}</div></div>
          <div><span class="${o.status === 'delivered' ? 'badge-yes' : 'badge-info'}">${o.status || 'pending'}</span></div>
        </div>`).join('')
  ...
  document.getElementById('auBody').innerHTML = `
    <div class="order-detail-block">
      ...
      <div class="row"><b>Name</b><span>${fullName}</span></div>
...
(async function boot() {
  ...
  if (page === 'category') {
    ...
    const ce = document.getElementById('catEyebrow');
  ...
  // PROFILE PAGE
  if (page === 'profile') { /* loaded after auth */ }
...
// Cart / wishlist openers
document.addEventListener('click', e => {
  const cartOpen = e.target.closest('#cartBtn');
  if (cartOpen) { openCart(); return; }

  const cartClose = e.target.closest('#cartClose');
  if (cartClose) { closeCart(); return; }

  const wishOpen = e.target.closest('#wishlistBtn');
  if (wishOpen) { openWishlist(); return; }

  const wishClose = e.target.closest('#wishlistClose');
  if (wishClose) { closeWishlist(); return; }

  const overlay = e.target.closest('#overlay');
  if (overlay) { closeCart(); closeWishlist(); return; }

  const searchToggle = e.target.closest('#searchToggle');
  if (searchToggle) { openSearch(); return; }

  const searchClose = e.target.closest('#searchClose');
  if (searchClose) { closeSearch(); return; }

  const searchOverlay = e.target.closest('#searchOverlay');
  if (searchOverlay && e.target === searchOverlay) { closeSearch(); return; }
  ...
  if (quickBtn) {
    ...
    if (!p) return;
    if (p.inStock === false) return toast('Out of stock');
  ...
  if (inc) {
    const found = cart.find(i => lineKey(i) === inc.dataset.cartInc);
    if (found) { found.qty++; saveCart(); renderCart(); }
    return;
  }
  const dec = e.target.closest('[data-cart-dec]');
  if (dec) {
    const idx = cart.findIndex(i => lineKey(i) === dec.dataset.cartDec);
    if (idx > -1) {
  ...
  if (wishRem) {
    wishlist = wishlist.filter(id => id !== wishRem.dataset.wishRemove);
    saveWishlist();
  ...
  // Detail size/color/qty
  const sizeBtn = e.target.closest('#detailSizes [data-size]');
  if (sizeBtn) {
    currentDetail.size = sizeBtn.dataset.size;
    document.querySelectorAll('#detailSizes .opt-btn').forEach(b => b.classList.toggle('active', b === sizeBtn));
    return;
  }
  const colorBtn = e.target.closest('#detailColors [data-color]');
  if (colorBtn) {
    currentDetail.color = colorBtn.dataset.color;
    document.querySelectorAll('#detailColors .color-btn').forEach(b => b.classList.toggle('active', b === colorBtn));
    return;
  }
  if (e.target.id === 'qtyMinus') {
    if (currentDetail.qty > 1) currentDetail.qty--;
    document.getElementById('qtyValue').textContent = currentDetail.qty;
    return;
  }
  ...
  if (e.target.closest('#detailWishBtn')) {
    if (currentDetail.product) toggleWishlist(currentDetail.product.id);
    return;
  }
  const thumb = e.target.closest('[data-thumb]');
  if (thumb) {
    const idx = Number(thumb.dataset.thumb);
    const img = currentDetail.images[idx];
    if (img) {
      document.getElementById('detailMedia').innerHTML = `<img src="${img}" alt="Product">`;
      document.querySelectorAll('.detail-thumbs .thumb').forEach((t, i) => t.classList.toggle('active', i === idx));
    }
    return;
  }
...
document.addEventListener('click', e => {
  ...
  if (s) {
    ...
    document.querySelectorAll('#starPicker span').forEach(sp => {
      sp.classList.toggle('active', Number(sp.dataset.star) <= selectedStar);
    });
  }
});
...
if (checkoutForm) checkoutForm.addEventListener('submit', async e => {
  ...
  const ok = [
    ...
    setErr(phoneEl, !isPhoneOk(phoneEl.value)),
    setErr(cityEl, cityEl.value.trim().length < 2),
    setErr(distEl, distEl.value.trim().length < 2),
    setErr(provEl, !provEl.value),
    setErr(addrEl, addrEl.value.trim().length < 5)
  ].every(Boolean);
...
document.querySelectorAll('.admin-tab').forEach(tab => {
  tab.addEventListener('click', () => {
    ...
    ['products', 'orders', 'users'].forEach(p => {
      const el = document.getElementById('panel-' + p);
      if (el) el.hidden = which !== p;
    });
  });
});
...
if (document.getElementById('exportUsers')) document.getElementById('exportUsers').addEventListener('click', () => {
  ...
  const rows = allUsers.map(u => [
    ...
    u.phone || '', u.city || '', u.district || '', u.province || '', u.provider || '',
  ...
  if (user) {
    try {
      ...
      if (pending && cart.length) {
        setTimeout(() => {
          toast('Placing your pending order…');
          placeOrder(user, pending);
        }, 800);
      }
    } catch (e) {}
  }
